...
// --- Google-only login: worker access requests + link resets ---------------
// Workers/designers sign in with Google (see lib/auth/session.ts). Someone
// not yet in the system files a request from /factory; a supervisor/boss
// approves it here, either creating a new worker or attaching the requester's
// Google account to an existing worker (e.g. one who forgot their old PIN).

export async function approveWorkerRequest(input: {
  requestId: string;
  existingWorkerId?: string;
  station?: string;
}): Promise<Result> {
  const session = await requireWorkerSecurity();
  const admin = createAdminClient();
  ...
  let workerId = input.existingWorkerId;
  if (workerId) {
    const { data: taken } = await admin
      ...
      .maybeSingle();
    if (taken) {
      return { ok: false, error: "That worker already has a Google account — reset it first." };
    }
  } else {
    const { data: created, error } = await admin
      .from("workers")
      .insert({ name: request.name, station: input.station?.trim() || null, pin_hash: null })
      .select("id")
      .single();
    if (error || !created) return { ok: false, error: error?.message ?? "Could not create worker." };
    workerId = created.id;
  }

  const { error: linkError } = await admin
    .from("worker_identities")
    .insert({ auth_user_id: request.auth_user_id, worker_id: workerId });
  if (linkError) return { ok: false, error: linkError.message };

  await admin
    .from("worker_access_requests")
    .update({ status: "approved", decided_by: session.userId, decided_at: new Date().toISOString() })
    .eq("id", request.id);
...
export async function rejectWorkerRequest(requestId: string): Promise<Result> {
  const session = await requireWorkerSecurity();
  const admin = createAdminClient();
  const { error } = await admin
    .from("worker_access_requests")
    .update({ status: "rejected", decided_by: session.userId, decided_at: new Date().toISOString() })
    .eq("id", requestId)
    .eq("status", "pending");
  if (error) return { ok: false, error: error.message };
  revalidatePath("/dashboard/workers");
...
// Detach a worker's/designer's Google account so they can connect a different
// one (lost account, new email) — they re-link with their PIN, so set one
// first if they don't have one.
export async function resetWorkerGoogle(workerId: string): Promise<Result> {
  await requireWorkerSecurity();
  const admin = createAdminClient();
  const { error } = await admin.from("worker_identities").delete().eq("worker_id", workerId);
  if (error) return { ok: false, error: error.message };
  revalidatePath("/dashboard/workers");
  return { ok: true };
}

export async function resetDesignerGoogle(designerId: string): Promise<Result> {
  await requireWorkerSecurity();
  const admin = createAdminClient();
  const { error } = await admin.from("designer_identities").delete().eq("designer_id", designerId);
  if (error) return { ok: false, error: error.message };
  revalidatePath("/dashboard/designers");
  return { ok: true };
}
